...
const Firestore = (props) => {
  ...
  React.useEffect(() => {
    const obtenerDatos = async () => {
      try {
        const data = await db.collection(props.user.uid).limit(2).orderBy('fecha').get()
    ...
    <div>
      <div className="row">
        <div className="col-md-6">
          ...
          <button className="btn btn-info btn-block mt-2 btn-sm" disabled={desactivar} onClick={() => siguiente()}>
          Más tareas...
          </button>
        </div>
        <div className="col-md-6">
